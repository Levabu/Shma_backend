...
class UsersDAO {
  ...
  static async getUsersBySearch(searchFor) {
    try {
      return await db.query(
        `SELECT * FROM ${this.table} WHERE userName LIKE '%${searchFor}%' OR (CONCAT(firstName, ' ', lastName)) LIKE '%${searchFor}%';`
      );
    } catch (error) {
      console.log("Unable to get users", error);
      return { error };
  ...
  static async getUsersByIds(arr) {
    try {
      ...
      return await db.query(
        `SELECT * FROM ${this.table} WHERE id IN ${idsString};`
      );
    } catch (error) {
      console.log("Unable to get users", error);
      return { error };
  ...
  static async addUser(userName, firstName, lastName, hashedPassword) {
    try {
      await db.query(
        `INSERT INTO ${this.table} (userName, firstName, lastName, password) VALUES ("${userName}", "${firstName}", "${lastName}", "${hashedPassword}");`
      );
    } catch (error) {
